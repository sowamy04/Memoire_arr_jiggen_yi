...
class UserService {
    ...
    public function __construct(UserPasswordEncoderInterface $encoder, DenormalizerInterface $denormalize, EntityManagerInterface $manager, SimpleUserRepository $simpleUserRepo, AdminRepository $adminRepository)
    {
        $this->dn = $denormalize;
        $this->encode = $encoder;
        $this->manage = $manager;
    ...
    public function modification_user($request, $class, $id)
    {
        ...
        if ($class == "App\\Entity\\SimpleUser"){
            ...
            ;
            if ($password) {
                $simpleUser->setPassword($this->encode->encodePassword($simpleUser, $password));
             }
        }
        else{
